...
瀏覽器端不放 Groq、Gemini 或其他 AI provider 的 API key。
...
- Microsoft Edge
- Node.js 20 或更新版本
- 一個新的 AI provider API key

重要：原始專案曾把 Groq key 寫進公開 Git history。那把 key 必須視為洩漏並撤銷，不能繼續使用；請建立新 key。
...
看到測試通過後，在 api 資料夾建立 .dev.vars。這個檔案只留在本機，絕對不要 commit。

最簡單的 Groq 設定：

    GROQ_API_KEY=貼上你新建立的key
    GROQ_MODEL=openai/gpt-oss-120b
    EXTENSION_TOKEN=請換成你自己的長隨機字串
    ALLOWED_ORIGIN=*

如果你使用 Gemini：

    GEMINI_API_KEY=貼上你的key
...
第一次執行可能會需要安裝 Wrangler。成功後，通常會看到本機網址：
...
- 只設定你實際要用的 provider 即可。
- provider 沒有 key 就不會啟用。
- model ID 可以透過 *_MODEL 更換；如果 provider 回報 model 不存在，請改成該 provider 控制台目前可用的 model ID，然後重新啟動 Wrangler。
- 不要把 .dev.vars 貼到 GitHub、Issue、PR 或聊天訊息。

## 4. 將 Extension 掛到 Edge
...
- .dev.vars 是否放在 api/.dev.vars。
- 是否至少有一個正確的 provider key。
- key 是否因為包含空白或引號而失效。
- 修改 .dev.vars 後是否重啟 Wrangler。
...
模型名稱是可替換設定，不要假設固定 model 永遠存在。

修改：

    GROQ_MODEL=目前 provider 控制台可用的model
    # 或
    GEMINI_MODEL=目前 provider 控制台可用的model
...
- 你要啟用的 provider key，例如 GROQ_API_KEY 或 GEMINI_API_KEY
